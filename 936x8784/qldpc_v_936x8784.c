...
uint8_t qldcp_minsum(int32_t n_iterations, float* L, float alpha, int32_t S[M], int32_t est_error[N], int32_t* it) {
    ...
    int32_t i, j, ip, jp, hard_decision, sign, parity, vl = 0;
    float sum, sumv[N], temp[N], min, min1, min2, one = 1, minus_one = -1, R_abs, x;
    int32_t check[M] = {0};
    ...
    while(*it < n_iterations && !corrected) {
        ...
        for(i = 0; i < M; i++) {
            ...
            asm volatile("vsetvli t0, %0, e32, m1, ta, ma" :: "r"(vl));
            asm volatile("vfmv.v.f v31, %0" :: "f"(MIN));
            ...
            asm volatile("vle32.v v1, (%0);" :: "r"(&R[i * h_MAX_NNZ]));
            ...
            asm volatile("vse32.v v8, (%0);" ::"r"(&R_aux[i * h_MAX_NNZ]));
        ...
        asm volatile("vsetvli t0, %0, e32, m1, ta, ma" :: "r"(6));
        asm volatile("vfmv.v.f v31, %0" :: "f"(0));

        for(j = 0; j < N; j++) {

            asm volatile("vle32.v v1, (%0);" :: "r"(&o[j])); 
            asm volatile("vfredmin.vs v2, v1, v31");
            asm volatile("vfmv.f.s %0, v2" : "=f"(&sumv[j]));
        ...
        asm volatile("vsetvli t0, %0, e32, m1, ta, ma" :: "r"(108));

        asm volatile("vid.v v1");  // v1 = [0,1,2,...,99]
        asm volatile("vdivu.vx v1, v1, %0":: "r"(6));  // v1 = floor(i / 5)
        ...
        for(j = 0; j < N * 6; j+=108) {
            ...
            asm volatile("vluxei32.v v4, (%0), v1" :: "r"(&sumv[j/6]));
            asm volatile("vfsub.vv v5, v4, v3"); 
        
            asm volatile("vfmul.vv v6, v2, v5");  

            asm volatile("vluxei32.v v7, (%0), v1" :: "r"(&L[j/6]));
        ...
        asm volatile("vsetvli t0, %0, e32, m1, ta, ma" :: "r"(122));
